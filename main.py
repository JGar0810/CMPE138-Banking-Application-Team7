from __future__ import annotations

import bcrypt

from app.account_operations import AccountBankingService
from app.card_operations import CardBankingService
from app.db import get_connection
from app.loan_operations import LoanService


def ask_int(prompt: str, allow_blank: bool = False):
    while True:
        raw = input(prompt).strip()
        if allow_blank and raw == "":
            return None
        try:
            return int(raw)
        except ValueError:
            print("Please enter a whole number.")


def ask_amount(prompt: str, allow_zero: bool = False):
    while True:
        raw = input(prompt).strip()
        try:
            value = float(raw)
            if value < 0 or (not allow_zero and value == 0):
                raise ValueError
            return raw
        except ValueError:
            extra = " or 0" if allow_zero else ""
            print(f"Please enter a positive number{extra}.")


def print_result(result):
    print("\nSUCCESS" if result.ok else "\nERROR")
    print(result.message)
    if result.payload:
        for key, value in result.payload.items():
            print(f"- {key}: {value}")
    print()


def login(conn):
    while True:
        print("\n=== Welcome to Team7 Banking System ===")
        print("Type 'exit' to quit.")
        username = input("Username: ").strip()

        if username.lower() in ("exit", "quit", "q"):
            return None

        password = input("Password: ").strip()

        cursor = conn.cursor(dictionary=True)

        # Employee
        cursor.execute(
            """
            SELECT employee_id, login_id, role, status, password_hash, 'employee' AS user_type
            FROM Employee
            WHERE login_id = %s
            """,
            (username,),
        )
        user = cursor.fetchone()

        # Customer
        if user is None:
            cursor.execute(
                """
                SELECT customer_id, username AS login_id, 'customer' AS role, status, password_hash, 'customer' AS user_type
                FROM Customer
                WHERE username = %s
                """,
                (username,),
            )
            user = cursor.fetchone()

        cursor.close()

        if user is None:
            print("Invalid username or password.")
            continue

        if user["status"] != "active":
            print("Account not active.")
            continue

        if not bcrypt.checkpw(
            password.encode("utf-8"),
            user["password_hash"].encode("utf-8"),
        ):
            print("Invalid username or password.")
            continue

        print(f"\nLogin successful. Role: {user['role']}")
        return user


# ---------------- CUSTOMER ----------------
def customer_menu(conn, user):
    card_service = CardBankingService(conn)
    loan_service = LoanService(conn)

    while True:
        print("\n--- Customer Menu ---")
        print("1. Apply for Loan")
        print("2. Request Card")
        print("3. Report Card Lost")
        print("0. Logout")

        choice = input("Select: ").strip()

        if choice == "1":
            loan_amount = ask_amount("Loan amount: ")
            interest_rate = ask_amount("Interest rate (%): ")
            loan_terms = ask_int("Loan term (months): ")

            result = loan_service.apply_for_loan(
                user["customer_id"], loan_amount, interest_rate, loan_terms
            )
            print_result(result)

        elif choice == "2":
            account_id = ask_int("Account ID: ")
            card_type = input("Card type (debit/credit): ").strip()
            result = card_service.request_new_card(account_id, card_type)
            print_result(result)

        elif choice == "3":
            card_id = ask_int("Card ID (blank for number): ", allow_blank=True)
            card_number = None
            if card_id is None:
                card_number = input("Card number: ")

            result = card_service.report_card_lost(card_id, card_number)
            print_result(result)

        elif choice == "0":
            break

        else:
            print("Invalid option.")


# ---------------- TELLER ----------------
def teller_menu(conn, user):
    account_service = AccountBankingService(conn)

    while True:
        print("\n--- Teller Menu ---")
        print("1. Deposit")
        print("2. Withdrawal")
        print("3. Open Account")
        print("4. Close Account")
        print("5. Transfer Funds")
        print("0. Logout")

        choice = input("Select: ").strip()

        if choice == "1":
            account_id = ask_int("Account ID: ")
            amount = ask_amount("Amount: ")
            result = account_service.process_deposit(account_id, amount, user["employee_id"])
            print_result(result)

        elif choice == "2":
            account_id = ask_int("Account ID: ")
            amount = ask_amount("Amount: ")
            result = account_service.process_withdrawal(account_id, amount, user["employee_id"])
            print_result(result)

        elif choice == "3":
            customer_id = ask_int("Customer ID: ")
            account_type = input("Type: ")
            result = account_service.open_account(customer_id, account_type, 0, user["employee_id"])
            print_result(result)

        elif choice == "4":
            account_id = ask_int("Account ID: ")
            result = account_service.close_account(account_id)
            print_result(result)

        elif choice == "5":
            from_id = ask_int("From: ")
            to_id = ask_int("To: ")
            amount = ask_amount("Amount: ")
            result = account_service.transfer_funds(from_id, to_id, amount, user["employee_id"])
            print_result(result)

        elif choice == "0":
            break


# ---------------- LOAN OFFICER ----------------
def loan_officer_menu(conn, user):
    loan_service = LoanService(conn)

    while True:
        print("\n--- Loan Officer Menu ---")
        print("1. Review Loans")
        print("2. Approve Loan")
        print("3. Reject Loan")
        print("0. Logout")

        choice = input("Select: ").strip()

        if choice == "1":
            print_result(loan_service.review_loans())

        elif choice == "2":
            loan_id = ask_int("Loan ID: ")
            print_result(loan_service.approve_loan(loan_id, user["employee_id"]))

        elif choice == "3":
            loan_id = ask_int("Loan ID: ")
            print_result(loan_service.reject_loan(loan_id, user["employee_id"]))

        elif choice == "0":
            break


# ---------------- MANAGER ----------------
def manager_menu(conn, user):
    account_service = AccountBankingService(conn)
    loan_service = LoanService(conn)

    while True:
        print("\n--- Manager Menu ---")
        print("1. Deposit")
        print("2. Withdrawal")
        print("3. Open Account")
        print("4. Close Account")
        print("5. Transfer")
        print("6. Review Loans")
        print("7. Approve Loan")
        print("8. Reject Loan")
        print("0. Logout")

        choice = input("Select: ").strip()

        if choice == "1":
            print_result(account_service.process_deposit(ask_int("Account: "), ask_amount("Amount: "), user["employee_id"]))

        elif choice == "2":
            print_result(account_service.process_withdrawal(ask_int("Account: "), ask_amount("Amount: "), user["employee_id"]))

        elif choice == "3":
            print_result(account_service.open_account(ask_int("Customer: "), input("Type: "), 0, user["employee_id"]))

        elif choice == "4":
            print_result(account_service.close_account(ask_int("Account: ")))

        elif choice == "5":
            print_result(account_service.transfer_funds(ask_int("From: "), ask_int("To: "), ask_amount("Amount: "), user["employee_id"]))

        elif choice == "6":
            print_result(loan_service.review_loans())

        elif choice == "7":
            print_result(loan_service.approve_loan(ask_int("Loan ID: "), user["employee_id"]))

        elif choice == "8":
            print_result(loan_service.reject_loan(ask_int("Loan ID: "), user["employee_id"]))

        elif choice == "0":
            break


# ---------------- MAIN ----------------
def main():
    conn = get_connection()

    while True:
        user = login(conn)

        if user is None:
            print("Goodbye.")
            break

        role = user["role"]

        if role == "customer":
            customer_menu(conn, user)
        elif role == "teller":
            teller_menu(conn, user)
        elif role == "loan_officer":
            loan_officer_menu(conn, user)
        elif role == "manager":
            manager_menu(conn, user)


if __name__ == "__main__":
    main()