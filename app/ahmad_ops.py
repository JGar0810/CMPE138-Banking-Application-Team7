...
class AhmadBankingService:
    ...
    def open_account(
        ...
        try:
            ...
            log_action(
                ...
                "open_account",
                f"success account_id={account_id} type={account_type} opening_deposit={deposit}",
            )
            return OperationResult(
                True,
                f"Account {account_id} opened successfully.",
                {"account_id": account_id, "balance": str(deposit), "account_type": account_type},
            )
        except Exception as exc:
            self.conn.rollback()
    ...
    def process_deposit(
        ...
        try:
            ...
            return OperationResult(
                True,
                f"Deposit complete. New balance: ${new_balance}",
                {"transaction_id": transaction_id, "new_balance": str(new_balance)},
            )
        except Exception as exc:
            self.conn.rollback()
    ...
    def process_withdrawal(
        ...
        try:
            ...
            if current_balance < amount_dec:
                raise BankingError(
                    f"Insufficient funds. Current balance is ${current_balance.quantize(MONEY_PLACES)}."
                )
            ...
            return OperationResult(
                True,
                f"Withdrawal complete. New balance: ${new_balance}",
                {"transaction_id": transaction_id, "new_balance": str(new_balance)},
            )
        except Exception as exc:
            self.conn.rollback()
    ...
    def close_account(self, account_id: int) -> OperationResult:
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) AS active_cards FROM Card WHERE account_id = %s AND status = 'active'",
                (account_id,),
            )
            ...
            cursor.execute(
                "UPDATE Account SET status = 'closed' WHERE account_id = %s",
                (account_id,),
            )
            self.conn.commit()
